...
public class script_smg : MonoBehaviour
{
    [Header("敵人血條設置")]
    int j;
    ...
    public int bulletsMag = 31; //一個彈匣的子彈數
    public int range = 100; //武器射程
    static public int bulletLeft = 30; //備彈
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(1)) // 1 表示滑鼠右鍵
        {
            ...
            if (Input.GetMouseButton(0)) ani.SetBool("isAimShoot", true);
        ...
        if (Physics.Raycast(pistol_muzzle.transform.position, -pistol_muzzle.transform.forward, out hitTreasure, 2.0f))
        {
            ...
            if (hitTreasure.collider.gameObject.name == "bullet_shotgun(Clone)")
            {
                if (Input.GetKey(KeyCode.E))
                {
                    if ((script_shotgun.bulletLeft + 10) <= 300)
                    {
                        script_shotgun.bulletLeft += 10;
                    }
                    else script_shotgun.bulletLeft = 300;
                    bulletsMagTextUI2.text = " " + script_shotgun.bulletLeft;
    ...
    public void UpdateAmmoUI()
    {
        ...
        bulletsMagTextUI3.text = " " + bulletLeft;
    }
